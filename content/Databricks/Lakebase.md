...
- **Consort: open-source agentic TDD framework on Lakebase branching** (Sep 9, 2026) — built by Databricks Field Engineering, not a core platform feature. Consort uses Lakebase's copy-on-write database branching (constant-time, regardless of data size) to give an AI coding agent a live branch of the real database as its test target instead of mocks, removing a long-standing reason mocked/hermetic tests existed in agentic dev workflows. Structures the work as a spec-first design lane plus a red/green/refactor build lane, with distinct agent roles (product owner, spec author, architect reviewer, DBA, test strategist, navigator/driver) coordinated by a deterministic (non-agentic) "conductor" state machine that holds human-approval gates; the code-writing role cannot edit tests to make them pass, and each feature's artifacts persist in a shared directory contract between roles to avoid agents losing the mission over long sessions. Includes a VS Code plugin (paired Git + Lakebase branch view, diffs) and an observability dashboard of each agent turn. Open source and community-supported (not an SLA-backed product). [Source](https://www.databricks.com/blog/introducing-consort-test-driven-development-branching-database) / [Source](https://github.com/databricks-solutions/consort)
- **Reference implementation: durable agents with Temporal + Lakebase** (Sep 8, 2026) — a Databricks-published reference implementation (loan-underwriting agent) pairs Temporal for durable workflow execution (replay-based recovery, per-operation retries, long human-review waits) with Lakebase Postgres for queryable operational state (run status, evidence, tool calls, review decisions). Unity Catalog policy reaches the agent via a Lakebase synced table; Lakebase Change Data Feed (Public Preview) can publish operational writes back to Unity Catalog-managed Delta history tables for audit. [Source](https://www.databricks.com/blog/build-durable-agents-temporal-and-lakebase)
