...
**First logged:** 2026-09-08 · **Last updated:** 2026-09-22 · **Status:** active — standards/consolidation
...
This window adds a fourth distribution channel on top of chat-client plugin (Sigma/ChatGPT), first-party connector catalog (Databricks' managed MCP connectors), and bare protocol-level GA (Qlik/Strategy/Fabric IQ's own MCP servers): **marketplace listing**. Qlik's already-GA MCP Server is now separately listed in **AWS Marketplace** (reaching Amazon Bedrock environments) and the **Databricks Marketplace** (purchasable via existing Databricks Universal Commits, reachable from inside Databricks' own agent experience) — an event dated Sep 2 but only surfaced into this vault Sep 21. That's a distinct move from a dedicated plugin or a first-party connector: it puts an already-built MCP server directly into the commerce/discovery layer of two rival platforms, letting a customer buy access to Qlik's governed semantic layer with spend they've already committed to AWS or Databricks, rather than standing up a new vendor relationship or integration from scratch. If this catches on, the next round of this pattern won't be "does vendor X have an MCP server" (settled) or "does vendor X have a dedicated chat-client plugin" (still Sigma-shaped and rare) — it'll be "is vendor X's MCP server purchasable inside the marketplaces I already spend through." Worth watching whether Strategy, Sigma, or Snowflake follow with their own marketplace listings, and whether Databricks or AWS ever lists a *competitor's* semantic layer this way again (notable that Databricks Marketplace is doing so here, for a company that competes with Genie/Unity Catalog's own semantic ambitions).

This window adds a seventh vendor, and a genuinely new kind of entrant: **Alteryx**, historically a data-prep/workflow-automation tool rather than a BI platform or warehouse — the first vendor in this vault's coverage to reach for MCP from that layer of the stack. Alteryx's Sep 9 announcement ships an **MCP Server** (any AI agent can find data, build multi-step solutions, and turn them into governed, repeatable Alteryx workflows, inheriting Alteryx's own auth/RBAC/audit trail) alongside a **ChatGPT plugin** — Alteryx Insights for OpenAI, shipped through the ChatGPT Plugin Directory — making Alteryx the second vendor after Sigma (Sep 11) to productize its MCP exposure as a dedicated chat-client plugin rather than leaving it as a bare protocol connection. That the same "chat-client plugin" distribution channel now has two independent confirming vendors, a month apart and a full category apart (BI vs. data-prep), suggests it's becoming a standard go-to-market move for any vendor with an MCP server, not a Sigma-specific bet. The bigger tell, though, is which vendor is doing this: MCP-as-semantic-exit-door was already confirmed baseline infrastructure across warehouses (Databricks, Snowflake) and BI incumbents (Qlik, Strategy, ThoughtSpot-adjacent); Alteryx is proof the same expectation has now reached the data-prep/ETL layer too — any vendor sitting on governed enterprise data, regardless of where it sits in the stack, is now expected to expose it to agents via MCP. Worth watching whether this reaches ingestion/orchestration vendors (Fivetran, dbt Labs) next.
...
- Qlik MCP Server expands to AWS Marketplace and Databricks Marketplace (event dated Sep 2, surfaced in this vault Sep 21) — [Source](https://www.globenewswire.com/news-release/2026/09/02/3355133/0/en/qlik-expands-mcp-availability-across-aws-and-databricks-marketplace-bringing-trusted-business-context-to-agents.html)
- Alteryx MCP Server and ChatGPT plugin (Alteryx Insights for OpenAI), Sep 9, surfaced in this vault Sep 22 — [Source](https://www.prnewswire.com/news-releases/alteryx-launches-new-ai-capabilities-to-bring-governed-analytics-anywhere-work-happens-302872761.html)
...
- 2026-09-21: added Qlik's MCP Server marketplace expansion (AWS Marketplace, Databricks Marketplace) as a fourth productization channel — commerce/discovery-layer distribution alongside chat-client plugins and first-party connector catalogs, and notable for Databricks Marketplace carrying a semantic-layer competitor's own MCP server.
- 2026-09-22: added Alteryx's MCP Server + ChatGPT plugin (Sep 9, backfilled) as a seventh confirming vendor and the first from outside BI/warehouse — a data-prep/ETL vendor now building the same MCP-exposed governed layer, and a second confirming instance of the chat-client-plugin distribution channel after Sigma.
...
**First logged:** 2026-09-11 · **Last updated:** 2026-09-22 · **Status:** active — escalation
...
This window adds a fourth vendor's confirming signal, and corrects a gap in this observation's own evidence base: **Snowflake CoWork Automations** — the recurring-report scheduling feature already sitting in this vault's [[Snowflake CoWork]] note as Public Preview (Aug 6) but never previously cited in this observation — reached **General Availability on Sep 11**, only surfaced into this vault's tracking Sep 22. CoWork automations re-run a saved question on a schedule with fresh data and deliver a summary, key metrics, and a link back into the interactive report, configurable conversationally or from a dedicated tab. That means Snowflake was actually *first* among the four vendors this observation tracks to ship an always-on/scheduled capability — its Aug 6 Public Preview predates both Genie Code's Sep 1 GA and Amazon Quick's Sep 9 always-on push — this observation simply under-credited Snowflake until today because of a tracking gap, not because Snowflake was late to the pattern. With Automations now GA rather than Preview, three of the four vendors here (Databricks, Amazon, Snowflake) have moved background execution out of experimental status entirely: the pattern isn't just "vendors are trying always-on agents" anymore, it's "always-on agents have graduated to production-grade, generally-available features" across the category's biggest platforms.
...
- Snowflake CoWork Automations reach GA (Sep 11, backfilled into this vault Sep 22; Public Preview since Aug 6) — [Source](https://docs.snowflake.com/en/release-notes/2026/other/2026-09-11-cowork-automations-ga)

**Update log:**
- 2026-09-11: initial observation.
- 2026-09-22: added Snowflake CoWork Automations reaching GA (Sep 11) as a fourth vendor's confirming signal — also corrects this observation's evidence base, since Snowflake's Aug 6 Public Preview actually predates every other vendor's shipment already cited here but was never included until today.
...
- 2026-09-14: Quiet. Checked the full Sep 1–14 window in both changelogs. The only genuinely new item since the Sep 13 Big Picture run is DeepSeek V4.1 Flash reaching Unity Gateway (Sep 13 vault entry, event dated Sep 11) — a routine single-vendor model-catalog addition with no second connected signal, not pattern-worthy on its own. Sep 13 and Sep 14 were logged as quiet days in both the Databricks and Market Landscape changelogs themselves. Everything else surfaced this window (Genie Agents' data-source/conversation limit increases, minor dashboard/Ask Genie UI additions, Genie One backend steering and PowerPoint uploads, Hex's generative-app-iteration update) is either too minor/isolated to connect to a cross-cutting pattern or already fully captured under existing Active Observations (MCP as two-way fabric, agent memory convergence, always-on agents, citations-as-trust-layer, chat-as-app-builder, adaptive execution tiering, deep research as a second tier) with no new evidence beyond what's already cited there.
- 2026-09-15: Quiet. Checked the Sep 1–15 window in both changelogs. Sep 13, 14, and 15 are now three straight quiet days in both source logs — the Databricks changelog explicitly reports "no genuinely new items" for all three dates (the one exception, DeepSeek V4.1 Flash reaching Unity Gateway, is dated Sep 11 and was already assessed as an isolated, non-pattern-worthy signal in the Sep 14 Big Picture entry above), and the Market Landscape changelog reports the same three consecutive quiet days across its full tracked vendor list (Snowflake, ThoughtSpot, Sigma, Amazon Quick Suite, Fabric IQ, Tableau Next, Hex, Omni, Metabase, WrenAI, Looker/Conversational Analytics, Cube/AtScale/dbt/TextQL/Strategy). No new cross-cutting pattern and no fresh evidence for any of the eight existing Active Observations this window.
- 2026-09-16: Quiet. Checked the Sep 3–16 window in both changelogs (10-14 day lookback). Three genuinely new items surfaced today, none pattern-worthy on their own and none connecting to a second signal: **Genie visualizations in Slack replies** (Public Preview, Databricks, dated Sep 14) is a narrow output-channel parity fix, not an escalation in capability. **Snowflake Cortex Agents/CoWork now return partial answers instead of hard-failing when a role lacks privilege on one of several configured tools** (a corrected two-week-old gap, event dated Sep 2) is a genuine governance nuance — multi-tool agents quietly maturing past all-or-nothing execution — but it's a single-vendor signal with no confirmed equivalent or contrast elsewhere in the vault to pair it with (checked Genie One/Genie Agents' Sep 10 MCP connector GA and Unity Catalog/Governance Hub notes — no documented equivalent behavior, but silence in Databricks' own docs isn't evidence of an actual gap worth calling whitespace). **Omni's week-of-Sep-7 batch** (find/open routines from chat, pagination) is minor and already the second correction to this same changelog gap this month. Everything else in the window (Genie One's Sep 10 batch of minor features, Genie Agents' capacity increases, the Sep 3/6/11 model-catalog additions to Unity Gateway, ThoughtSpot's Sep 9 release) was already synthesized into the eight Active Observations above or assessed as non-pattern-worthy in the Sep 14 entry — no fresh evidence for any of them this run.
- 2026-09-17: Quiet. No dated Sep 17 entry exists yet in either the [[Databricks/Updates|Databricks]] or [[Market Landscape/Updates|Market Landscape]] changelog at run time, so today's run had no fresh raw material beyond the Sep 3–16 window — and that window is the same one the Sep 16 Big Picture run already checked and fully synthesized (including logging its own quiet-day line above). Re-read both changelogs in full back to Sep 3 and re-checked each of the eight Active Observations for anything not yet cited — nothing new. No new pattern, no update to any existing observation.
- 2026-09-19: Quiet. No dated Sep 19 entry exists yet in either changelog at run time; newest entries in both are dated Sep 18. The [[Databricks/Updates|Databricks]] Sep 18 entry reports no genuinely new in-scope items. The [[Market Landscape/Updates|Market Landscape]] Sep 18 entry's one finding — Amazon's new energy-industry workflows for Quick (event dated Sep 17) — is the same item already cited as the second data point in the "From answers to actions" observation added by yesterday's (Sep 18) run; nothing further to add. Re-read the full Sep 5–18 window in both changelogs and re-checked all ten existing Active Observations — no new cross-cutting pattern, no update to any existing observation.
