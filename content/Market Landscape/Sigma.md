...
- **2026-09-18** — A wide Beta batch focused on AI/agent tooling: **migration skills for AI assistants** (Beta) — Claude Code, Cursor, Cortex Code, or Codex can use per-source-tool Sigma skills to rebuild dashboards/reports/data models from another BI tool as Sigma documents, with a read-only assessment skill to inventory existing content first; new **`sigma-workbooks`/`sigma-reports` skills** for AI assistants authoring the code representation of a workbook/report; **Sigma Assistant in build mode moves to a code-first architecture** (Beta) — more accurate/consistent outputs, faster builds with lower token usage, better data-source search/retrieval, plus broad new native feature support (chat elements with custom-agent creation, embed/value-list/single-row-container elements, export actions, file-upload fields); **chat history for Sigma agents** (Beta), configurable by admins; and new **API endpoints to list and run Sigma agents programmatically** (Beta) — the same agents used interactively can now be called from external business processes, plus **manage workbooks/reports from a JSON/YAML code representation** via the API (Beta). Also shipped: audit-log event categories for AI settings and AI conversations, admin ability to view a document's code representation, and GA of app templates (10 prebuilt apps with sample data/agents) and choosing related datasets when migrating to a data model. Extends Sigma's "workbooks as code" direction (Jul 2026) into a full agent-as-API and AI-assisted-migration story. [Source](https://community.sigmacomputing.com/t/whats-new-in-sigma-september-18-2026/7246)
