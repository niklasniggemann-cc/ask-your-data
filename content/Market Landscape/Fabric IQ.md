...
- **2026-08-31** (effective Aug 26, 2026) — **Fabric data agent's Assistants API retired; MCP endpoint becomes the primary programmatic path.** OpenAI is shutting down the Assistants API on Aug 26, 2026; since the Fabric data agent's orchestration layer was built on it, direct API callers must migrate to the Fabric OpenAI Responses client or, per Microsoft's recommendation, the data agent's own MCP endpoint. SDK and Fabric-portal users are migrated automatically. A distinct thread from the Copilot-in-Power-BI retirement below (different integration, same Aug 26 effective date) — reinforces the same MCP-as-consumption-surface pattern as the Ontology MCP server. [Source](https://learn.microsoft.com/en-us/fabric/data-science/consume-data-agent-python)
