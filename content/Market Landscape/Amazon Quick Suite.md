...
**2026-09-17** — **New energy-industry workflows**: a set of ready-to-use AI workflows purpose-built for energy and utilities customers (grid planning, asset management, drilling exploration, production monitoring, compliance tracking, energy trading), powered by MCP-connected integrations from 15+ energy-industry partners (Accenture, EPAM, Hitachi Digital Services, IBM, Infosys, S&P Global, Seeq, Slalom, and others). Lets energy customers pull data across systems into a single Quick interface that can make recommendations and implement solutions, compressing multi-week manual coordination into hours. Available to Quick users in the energy sector via account-manager request. A vertical packaging of existing MCP-connector/agent capabilities for one industry rather than a new underlying platform capability. [Source](https://press.aboutamazon.com/aws/2026/9/aws-announces-new-energy-industry-workflows-for-amazon-quick)

**2026-09-09** — **Always-on agents, sharper activity feed, and enterprise controls**: scheduled tasks and monitoring agents now run in the cloud and keep delivering results to a user's feed even when their laptop is closed, rather than stopping when the user steps away. The activity feed gets top-level filters, an improved catch-up view, a daily briefing refreshing three times a day, and search across up to seven days of feed data. Administrators gain mobile device management support, custom per-user permissions, and Microsoft Purview data loss prevention integration. Teams can now share files and publish agents/skills for others to discover and install, and browse a catalog of official skills. Quick spaces and Quick apps now run natively on desktop, any indexed local folder can become a searchable space, and inline citations let users verify every answer against its source. Every subscriber also gets more agent hours per month (Professional 4→8, Enterprise 8→18), combined into one simpler allotment. A substantial cross-cutting release spanning always-on execution, governance, and trust/citations in one drop. [Source](https://aws.amazon.com/about-aws/whats-new/2026/09/amazon-quick-always-on-agents-sharper-feed-enterprise-controls/)
