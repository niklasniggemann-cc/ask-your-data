...
**2026-09-09** — ThoughtSpot Cloud 26.9.0.cl release (now available): **Spotter Memory reaches GA** (on by default; memory can now be generated from Liveboards as well as conversations); a **Spotter 3 visual redesign** (more streamlined/conversational, no change to underlying reasoning); **multi-Model search in Spotter** (Beta) — one conversation can now span multiple Models at once; a **"Remember this" button** (replacing "+ Add to Coaching") that previews the exact memory it will add before committing; an **AI context redesign** letting authors prompt AI-context creation from a Model's Spotter optimization tab, auto-generated whenever Spotter is enabled; and **Spreadsheet on live data** (Early Access) — an editable grid (70+ formula functions, sort/filter/conditional formatting) running live queries against the governed model rather than an exported copy, saveable as a governed Answer. Also: pin a chat in the Spotter side menu, quick actions on any answer, and aggregate-aware query execution GA. [Source](https://docs.thoughtspot.com/cloud/26.9.0.cl/notes.html)

**2026-08-18** — ThoughtSpot Cloud 26.8.0.cl release: **SpotterModel reached GA** (auto-generates optimized semantic Models — table selection, joins, column selection — with review/customize at each step); **Spotter Analysts** entered Early Access (see Core Capabilities above); **Spotter starter prompts** let admins configure suggested first questions to guide new users; **Share chats** (Early Access) lets a user hand off a full Spotter conversation thread — questions, iterations, and analysis — rather than a screenshot; **SpotterViz insight tiles** (Early Access) add AI-generated text analysis tiles to a Liveboard that re-run against live data on every load. Same release also adds **exportable Spotter training memory** — the rules and recipes Spotter has learned for a data model can now be exported and imported across environments via REST API, letting a team carry accumulated tuning from dev to production or seed a new data model instead of retraining from scratch. [Source](https://docs.thoughtspot.com/cloud/26.8.0.cl/notes)
