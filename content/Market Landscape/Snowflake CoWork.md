...
**2026-09-21** — **Cortex Agents Compact API (Preview)**: a new `agent:compact` endpoint summarizes a conversation and passes the compact representation into subsequent `agent:run` requests, reducing token consumption and helping long-running agent conversations stay within the model's context window. [Source](https://docs.snowflake.com/en/release-notes/all-release-notes)

**2026-09-16 (GA)** — **Cortex Agents object enhancements**: temporary agents (`CREATE TEMPORARY AGENT`/`CREATE TEMP AGENT`) for session-scoped testing and ephemeral pipelines; secure agents, preventing non-owner roles from retrieving an agent's specification through metadata surfaces; and `COPY GRANTS` support on `CREATE OR REPLACE AGENT`, preserving explicit privileges (except ownership) when an agent is replaced. [Source](https://docs.snowflake.com/en/release-notes/2026/other/2026-09-16-cortex-agents-object-enhancements-ga)

**2026-09-11 (GA, was Public Preview 2026-08-06)** — **Automations in Snowflake CoWork reach General Availability**: CoWork re-runs a saved question on a schedule with fresh data and emails a summary, key metrics, and a link back into the full interactive report; set up conversationally or from a dedicated Automations tab, and listable, reschedulable, pausable, resumable, or deletable either way. [Source](https://docs.snowflake.com/en/release-notes/2026/other/2026-09-11-cowork-automations-ga)

**2026-09-02** — **Runs proceed when some tools are inaccessible**: when the calling user's role lacks a privilege on one of an agent's configured Cortex Search/Cortex Analyst/MCP/skill tools, Cortex Agents and CoWork no longer fail the entire run with an HTTP 4XX error — the agent now answers with the tools the role can access and emits a `response.warning` for each inaccessible one. Configurable per-agent (`accept`/`reject`/`legacy`); `accept` is the new default. Lets a single agent serve users across different privilege levels instead of requiring one agent per privilege class. [Source](https://docs.snowflake.com/en/release-notes/bcr-bundles/un-bundled/bcr-2425)
