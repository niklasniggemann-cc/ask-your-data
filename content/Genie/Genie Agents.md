...
- **2026-09-17** (minor, log-only) — **Larger PDF uploads** (100 pages, up from 20), **summarize up to six months of agent activity** from the Monitor tab, **agents now use only explicitly attached data sources** (scoping/behavior fix), and a **fixed Analyze Agent Usage bug** (previously summarized only the last 20 messages). [Source](https://docs.databricks.com/aws/en/ai-bi/release-notes/2026)
- **2026-09-10** (minor) — **Increased data source limit**: each Genie Agent now supports up to 50 tables/views/metric views as data sources, up from 30. **Increased conversation limit**: each Genie Agent can now store up to 200,000 conversations, up from 10,000. [Source](https://docs.databricks.com/aws/en/genie-agents/set-up#tech-requirements)
- **2026-08-26** (backfilled) — **Submit prompts to a Genie Agent in Chat mode directly from Genie One**, and **cancel in-progress Agent mode responses through the API** — both shipped the same day as the already-logged "Download visualizations API reaches GA." [Source](https://learn.microsoft.com/en-us/azure/databricks/ai-bi/release-notes/2026)
- **2026-09-02** — **Genie Code can diagnose agent failures and manage agents in production**: beyond single-prompt creation, Genie Code can now review a specific conversation failure or benchmark run and propose context fixes, and can summarize an in-production agent's question topics and feedback trends to surface knowledge gaps — author review still required before saving suggestions. [Source](https://www.databricks.com/blog/expanding-genie-agents-deep-analysis-file-reasoning-and-more)
- **2026-08-27** — **Agent mode APIs reach GA** (previously Beta since Jul 23), and **Unity Catalog functions as agent tools**: agent authors can now attach both scalar and table-valued UC functions as Genie Agent tools directly, alongside the existing SQL-functions-for-logic configuration component. Also, **downloading visualizations via the Genie Conversation API reaches GA**. [Source](https://docs.databricks.com/aws/en/ai-bi/release-notes/2026)
- **2026-08-19** — **Single-prompt agent creation**: describing an agent's goal and pointing to a data/document source in one plain-language prompt (via [[Genie One]] or [[Genie Code]]) is now enough to generate a working Genie Agent, drawing on [[Unity Catalog]] context and the user's conversation — no manual table/instruction/example-SQL configuration required as a starting point. Databricks frames this as solving a context problem, not a prompting problem: agents reason over governed tables plus unstructured files (PDFs, Word docs, presentations, images) in [[Unity Catalog]] volumes, and quality now depends on the curated context behind the prompt more than the prompt's wording. Complements the existing UI/REST API/SDK creation methods above. [Source](https://www.databricks.com/blog/designing-effective-genie-agents-single-prompt)
...
- [[Semantic Layer]] — the architectural layer Genie Agents queries via Unity Catalog and Metric Views instead of raw tables
