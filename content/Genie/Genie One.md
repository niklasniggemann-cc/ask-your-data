...
- **2026-09-17** — **Skills support multiple files and folders**: a Genie One skill can now include multiple files and sub-folders (up to three folders deep, 50 files/folders per skill, text files only), extending the Jun 4 User skills feature beyond a single file. [Source](https://docs.databricks.com/aws/en/genie-one/chat#user-skills)
- **2026-09-16** (minor) — **Account-only user access (Beta)**: users in a registered identity provider can now access account-level Genie One without being assigned to a workspace or granted consumer access; eligible users are enrolled automatically, and Unity Catalog continues to govern what each user can see. [Source](https://docs.databricks.com/aws/en/genie-one/account-only-user-access)
- **2026-09-14** — **Genie visualizations in Slack replies** (Public Preview): when a Genie response includes a chart, the visualization now appears as an image directly in the Slack reply rather than text-only. Requires reinstalling the Databricks Genie app for Slack if already installed. [Source](https://docs.databricks.com/aws/en/genie-one/genie-slack#visualizations)
